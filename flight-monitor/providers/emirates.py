"""Emirates provider using Playwright browser automation.

Emirates operates via Dubai (DXB) hub. Economy includes 1 × 30kg checked bag.
"""
...
class EmiratesProvider(BaseProvider):
    name = "emirates"
    ...
    async def search_flights(
        ...
        logger.info("Emirates: searching %s→%s %s to %s", origin, destination, dep_str, ret_str)
        ...
        url = (
            f"https://www.emirates.com/ca/english/book/flights/"
            f"?from={origin}&to={destination}"
            f"&departing={dep_ddmmmyyyy}&returning={ret_ddmmmyyyy}"
            f"&pax=a:{passengers},c:0,i:0&class={cabin_code}"
        )
        ...
        try:
            ...
            if any(kw in page_text.lower() for kw in ["captcha", "verify", "blocked", "access denied"]):
                logger.warning("Emirates: access blocked")
                return []

            offers = await self._parse_results(page, origin, destination, dep_str, ret_str, passengers, cabin)
            logger.info("Emirates: found %d results for %s-%s", len(offers), dep_str, ret_str)

        except Exception as e:
            logger.error("Emirates search error: %s", e)
        finally:
            await page.close()
        ...
        try:
            ...
            for idx, ps in enumerate(prices[:10]):
                ...
                offer = FlightOffer(
                    ...
                    airline="Emirates", marketing_airline="Emirates",
                    booking_provider="Emirates",
                    source_website="emirates.com",
                    booking_url=f"https://www.emirates.com/ca/english/book/flights/?from={origin}&to={destination}",
                    is_airline_direct=True,
                    checked_bags_included=1,
                    baggage_status=BaggageStatus.VERIFIED_INCLUDED.value,
                    baggage_weight_kg=30,
                    baggage_info_source="Emirates economy baggage allowance (1 × 30kg)",
                    outbound=FlightLeg(stops=1, layover_airports=["DXB"], total_duration_minutes=dur_min or None),
                    inbound=FlightLeg(stops=1, layover_airports=["DXB"]),
                    verification_level=VerificationLevel.LIKELY.value,
                )
                offers.append(offer)
        except Exception as e:
            logger.warning("Emirates parsing error: %s", e)
