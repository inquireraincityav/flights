"""Air France provider using Playwright browser automation.

Air France operates via Paris Charles de Gaulle (CDG) hub. Economy includes 1 × 23kg checked bag on long-haul.
"""
...
class AirFranceProvider(BaseProvider):
    name = "air_france"
    ...
    async def search_flights(
        ...
        logger.info("Air France: searching %s→%s %s to %s", origin, destination, dep_str, ret_str)

        url = (
            f"https://www.airfrance.ca/search/offers?"
        ...
        try:
            ...
            if any(kw in page_text.lower() for kw in ["captcha", "verify", "blocked", "access denied"]):
                logger.warning("Air France: access blocked")
                return []

            offers = await self._parse_results(page, origin, destination, dep_str, ret_str, passengers, cabin)
            logger.info("Air France: found %d results for %s-%s", len(offers), dep_str, ret_str)

        except Exception as e:
            logger.error("Air France search error: %s", e)
        finally:
            await page.close()
        ...
        try:
            ...
            for idx, ps in enumerate(prices[:10]):
                ...
                offer = FlightOffer(
                    ...
                    airline="Air France", marketing_airline="Air France",
                    booking_provider="Air France",
                    source_website="airfrance.ca",
                    booking_url=f"https://www.airfrance.ca/search/offers?origin={origin}&destination={destination}",
                    is_airline_direct=True,
                    checked_bags_included=1,
                    baggage_status=BaggageStatus.VERIFIED_INCLUDED.value,
                    baggage_weight_kg=23,
                    baggage_info_source="Air France economy long-haul baggage (1 × 23kg)",
                    outbound=FlightLeg(stops=1, layover_airports=["CDG"], total_duration_minutes=dur_min or None),
                    inbound=FlightLeg(stops=1, layover_airports=["CDG"]),
                    verification_level=VerificationLevel.LIKELY.value,
                )
                offers.append(offer)
        except Exception as e:
            logger.warning("Air France parsing error: %s", e)
