"""Turkish Airlines provider using Playwright browser automation.

Turkish Airlines operates via Istanbul (IST) hub. Economy includes 1 × 30kg checked bag.
"""
...
class TurkishAirlinesProvider(BaseProvider):
    ...
    async def search_flights(
        ...
        logger.info("Turkish Airlines: searching %s→%s %s to %s", origin, destination, dep_str, ret_str)

        url = (
            f"https://www.turkishairlines.com/en-ca/flights/booking/"
            f"?origin={origin}&destination={destination}"
            f"&departureDate={dep_str}&returnDate={ret_str}"
            f"&adult={passengers}&child=0&infant=0"
            f"&cabinClass=ECONOMY&tripType=roundTrip"
        )
        ...
        try:
            ...
            if any(kw in page_text.lower() for kw in ["captcha", "verify", "blocked", "access denied"]):
                logger.warning("Turkish Airlines: access blocked")
                return []

            offers = await self._parse_results(page, origin, destination, dep_str, ret_str, passengers, cabin)
            logger.info("Turkish Airlines: found %d results for %s-%s", len(offers), dep_str, ret_str)

        except Exception as e:
            logger.error("Turkish Airlines search error: %s", e)
        finally:
            await page.close()
    ...
    async def _parse_results(self, page, origin, destination, dep_str, ret_str, passengers, cabin) -> List[FlightOffer]:
        ...
        try:
            ...
            price_pattern = re.compile(r'(?:C?\$|CAD|TRY|₺)\s*([\d,]+(?:\.\d{2})?)')
            ...
            for idx, ps in enumerate(prices[:10]):
                ...
                offer = FlightOffer(
                    ...
                    airline="Turkish Airlines", marketing_airline="Turkish Airlines",
                    booking_provider="Turkish Airlines",
                    source_website="turkishairlines.com",
                    booking_url=f"https://www.turkishairlines.com/en-ca/flights/booking/?origin={origin}&destination={destination}",
                    is_airline_direct=True,
                    checked_bags_included=1,
                    baggage_status=BaggageStatus.VERIFIED_INCLUDED.value,
                    baggage_weight_kg=30,
                    baggage_info_source="Turkish Airlines economy baggage allowance (1 × 30kg)",
                    outbound=FlightLeg(stops=1, layover_airports=["IST"], total_duration_minutes=dur_min or None),
                    inbound=FlightLeg(stops=1, layover_airports=["IST"]),
                    verification_level=VerificationLevel.LIKELY.value,
                )
                offers.append(offer)
        except Exception as e:
            logger.warning("Turkish Airlines parsing error: %s", e)
