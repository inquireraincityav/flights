"""Lufthansa provider using Playwright browser automation.

Lufthansa operates via Frankfurt (FRA) or Munich (MUC) hubs. Economy includes 1 × 23kg checked bag.
"""
...
class LufthansaProvider(BaseProvider):
    name = "lufthansa"
    ...
    async def search_flights(
        ...
        logger.info("Lufthansa: searching %s→%s %s to %s", origin, destination, dep_str, ret_str)

        url = (
            f"https://www.lufthansa.com/ca/en/flight-search?"
            f"origin={origin}&destination={destination}"
            f"&outbound-date={dep_str}&inbound-date={ret_str}"
            f"&adults={passengers}&cabin-class=economy"
        )
        ...
        try:
            ...
            if any(kw in page_text.lower() for kw in ["captcha", "verify", "blocked", "access denied"]):
                logger.warning("Lufthansa: access blocked")
                return []

            offers = await self._parse_results(page, origin, destination, dep_str, ret_str, passengers, cabin)
            logger.info("Lufthansa: found %d results for %s-%s", len(offers), dep_str, ret_str)

        except Exception as e:
            logger.error("Lufthansa search error: %s", e)
        finally:
            await page.close()
        ...
        try:
            ...
            for idx, ps in enumerate(prices[:10]):
                ...
                offer = FlightOffer(
                    ...
                    airline="Lufthansa", marketing_airline="Lufthansa",
                    booking_provider="Lufthansa",
                    source_website="lufthansa.com",
                    booking_url=f"https://www.lufthansa.com/ca/en/flight-search?origin={origin}&destination={destination}",
                    is_airline_direct=True,
                    checked_bags_included=1,
                    baggage_status=BaggageStatus.VERIFIED_INCLUDED.value,
                    baggage_weight_kg=23,
                    baggage_info_source="Lufthansa economy baggage allowance (1 × 23kg)",
                    outbound=FlightLeg(stops=1, layover_airports=["FRA"], total_duration_minutes=dur_min or None),
                    inbound=FlightLeg(stops=1, layover_airports=["FRA"]),
                    verification_level=VerificationLevel.LIKELY.value,
                )
                offers.append(offer)
        except Exception as e:
            logger.warning("Lufthansa parsing error: %s", e)
