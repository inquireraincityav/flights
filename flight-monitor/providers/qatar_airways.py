"""Qatar Airways provider using Playwright browser automation.

Qatar Airways operates via Doha (DOH) hub. Economy includes 1 × 30kg checked bag.
"""
...
class QatarAirwaysProvider(BaseProvider):
    name = "qatar_airways"
    ...
    async def search_flights(
        ...
        logger.info("Qatar Airways: searching %s→%s %s to %s", origin, destination, dep_str, ret_str)

        url = (
            f"https://www.qatarairways.com/en-ca/booking/flights.html?"
            f"from={origin}&to={destination}"
            f"&departing={dep_str}&returning={ret_str}"
            f"&adults={passengers}&children=0&infants=0"
            f"&class=Economy&type=return"
        )
        ...
        try:
            ...
            if any(kw in page_text.lower() for kw in ["captcha", "verify", "blocked", "access denied"]):
                logger.warning("Qatar Airways: access blocked")
                return []

            offers = await self._parse_results(page, origin, destination, dep_str, ret_str, passengers, cabin)
            logger.info("Qatar Airways: found %d results for %s-%s", len(offers), dep_str, ret_str)

        except Exception as e:
            logger.error("Qatar Airways search error: %s", e)
        finally:
            await page.close()
        ...
        try:
            ...
            for idx, ps in enumerate(prices[:10]):
                ...
                offer = FlightOffer(
                    ...
                    airline="Qatar Airways", marketing_airline="Qatar Airways",
                    booking_provider="Qatar Airways",
                    source_website="qatarairways.com",
                    booking_url=f"https://www.qatarairways.com/en-ca/booking/flights.html?from={origin}&to={destination}",
                    is_airline_direct=True,
                    checked_bags_included=1,
                    baggage_status=BaggageStatus.VERIFIED_INCLUDED.value,
                    baggage_weight_kg=30,
                    baggage_info_source="Qatar Airways economy baggage allowance (1 × 30kg)",
                    outbound=FlightLeg(stops=1, layover_airports=["DOH"], total_duration_minutes=dur_min or None),
                    inbound=FlightLeg(stops=1, layover_airports=["DOH"]),
                    verification_level=VerificationLevel.LIKELY.value,
                )
                offers.append(offer)
        except Exception as e:
            logger.warning("Qatar Airways parsing error: %s", e)
